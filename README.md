...
- 实现基于**卡尔曼滤波**的预测轨迹后处理；基于**时空联合交互优化**的变道博弈与 cut-in 识别以及基于**贝叶斯模型**的 cut-in 识别
- 实现基于 **A\* 搜索**的速度决策与**多 ref 联合优化**的速度规划，并基于 **AL-iLQR** 进行求解
- **舒适性泛化指标**由 **3.84 次/100km** 降至 **0.43 次/100km**，**安全接管泛化指标**由 **2.12 次/100km** 降至 **0.20 次/100km**，**变道成功率**由 **87%** 提升至 **98%**
- **荣誉**：试用期绩效 **A（前 5%）**、**2025 届优秀校招生**、**2025 Q3 最佳新锐奖**、**奇瑞 M32T 项目攻关之星**；职级由 **2-2 晋升至 2-3**

#### 基于三维 DP 搜索的时空联合参考轨迹生成（2025.11 - 2026.03）
- 在 **SLT 三维时空空间**中通过 **DP 搜索**，设计**横向偏移/平滑度代价**、**纵向速度跟踪/加加速度代价**及**障碍物碰撞 sigmoid 代价**；通过**加速度约束下的纵向可达域 + 最大横向位移可达域**对问题进行剪枝，直接求解时空联合最优轨迹
- **安全接管泛化指标**由 **0.20 次/100km** 降至 **0.10 次/100km**，**路口通行成功率**由 **94.2%** 提升至 **97.8%**
- **荣誉**：**2025 Q4 绩效 A（前 5%）**、**2025 年度绩效 A（前 5%）**、**2025 年度飞车新秀奖**、**一汽奔腾 E541 项目攻关之星**

#### 端到端模型及时空联合交互优化后处理（2026.03 - 至今）
- 基于**障碍物决策导向规划模型**输出自车**多模态轨迹**与他车决策意图及预测轨迹
- 以自车多模态轨迹为参考、他车横纵决策意图为**半平面纵向约束**与**三圆盘横向避让约束**的方向依据，构建**多决策联合优化问题**，并通过 **iLQR** 求解自车与关键障碍物的**联合运动学最优解**
- 以**安全/效率/导航/舒适性代价 + 模型置信度代价**加权评分，筛选最优场景轨迹执行
